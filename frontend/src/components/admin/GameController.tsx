...
export default function GameController({ sessionId, gameState }: { sessionId: string, gameState: any }) {
  ...
  useEffect(() => {
    fetchKeywords();
  }, [sessionId, gameState?.state]); // Refetch when state changes (e.g. round ends)

  // Auto select first team if waiting and not selected
  useEffect(() => {
    if (gameState?.state === 'WAITING' && !selectedTeamId && gameState?.teams?.length > 0) {
      setSelectedTeamId(gameState.teams[0].id);
    }
  }, [gameState, selectedTeamId]);

  // Auto select first available keyword
  useEffect(() => {
    if (keywords.length > 0) {
      const selected = keywords.find(k => k.id === selectedKeywordId);
      if (!selected || selected.is_used) {
        const nextAvailable = keywords.find(k => !k.is_used);
        if (nextAvailable) {
              ...
              {keywords.map((k, index) => (
                <button
                  ...
                  className={`text-left p-3 rounded border flex items-center justify-between transition-colors
                    ${k.is_used ? 'bg-gray-200 text-gray-500 cursor-not-allowed opacity-60' 
                  ...
                  {teams.filter((t: any) => t.id !== current_team.id).map((t: any) => (
                    <div key={t.id} className="border rounded p-2 flex flex-col gap-2 bg-gray-50">
                      ...
                      <div className="flex gap-2">
                        <button className="flex-1 bg-green-500 text-white text-sm py-1 rounded" onClick={() => triggerAction('steal', { steal_team_id: t.id, correct: true })}>ĐÚNG (+5)</button>
                        <button className="flex-1 bg-red-500 text-white text-sm py-1 rounded" onClick={() => triggerAction('steal', { steal_team_id: t.id, correct: false })}>SAI (0)</button>
                      </div>
                    </div>
                  ))}
                </div>
